...
def load_ml_model():
    ...
    model = yolov5.load(settings_server.WEIGHT_PATH)

    # set model parameters
    model.conf = 0.25
    model.iou = 0.45
    model.agnostic = False
    model.classes = settings_server.CLASSES_LIST
...
class VideoLoader:
    ...
    def load_local_vid(self, filename):
        ...
        vid = cv2.VideoCapture(os.path.join(
            settings_server.VIDEO_PATH, filename))
    ...
    def load_stream(self, streampath):
        ...
        vid = cv2.VideoCapture(os.path.join(
            settings_server.VIDEO_PATH, streampath))
        vid.set(cv2.CAP_PROP_BUFFERSIZE, 2)
        print("vid is:", vid)
        return vid
